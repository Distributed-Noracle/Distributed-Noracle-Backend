...
ENDPOINT="https://localhost:9082/distributed-noracle/v0.5.0"
...
curl -s --user noracle-example-smith:${AGENT_PW} -X GET --header 'Accept: application/json' "${ENDPOINT}"'/agents/'"${SPACE_CREATOR_AGENT_ID}"'/spacesubscriptions' --insecure | jq -r '.[].spaceId' | while read spaceId ; do
  for num in {1..50}; do
    ...
    curl -s -D - --user noracle-example-smith:${AGENT_PW} -X POST --header 'Content-Type: application/json' -d '{ "text": "'"${random}"'" }' "${ENDPOINT}"'/spaces/'"${spaceId}"'/questions' --insecure
  done
done
